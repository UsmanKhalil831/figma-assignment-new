...
import 'package:assignment/FishProducts.dart';
...
class BignSmallFishes extends StatefulWidget {
  const BignSmallFishes({super.key});

  @override
  State<BignSmallFishes> createState() => _BignSmallFishesState();
}

class _BignSmallFishesState extends State<BignSmallFishes> {
  List cartItems = [];
  List favItems = [];
  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Big & Small Fishes',style: TextStyle(color: Colors.white,fontSize: 20),),    
          actions: [
            Wrap(children: [
              Stack(
                children: [
      ...
      child: Container(alignment: Alignment.center,margin: const EdgeInsets.fromLTRB(3,0,3,0),
          decoration:const BoxDecoration(color: Colors.amber,borderRadius: BorderRadius.all(Radius.elliptical(50, 50)) ),child:const Text('Big Fishes',style: TextStyle(fontSize: 15,),)),
             ...
             Expanded(
              child: 
              GridView.builder(itemCount: FishProducts.length,
                  ...
                  return  
                    Container(padding: const EdgeInsets.all(30),margin: const EdgeInsets.fromLTRB(10, 0, 10, 0),decoration: const BoxDecoration(color:Color(0xFFE0E2EE),borderRadius: BorderRadius.only(topLeft: Radius.circular(10),bottomLeft: Radius.circular(10),topRight: Radius.circular(10),bottomRight: Radius.circular(10))),
                      child: Column(children: [
                        Container(child: Text(FishProducts[index]['name']),),
                        const SizedBox(height: 5,),
                        Container(child: Text(FishProducts[index]['price'])),
                        const SizedBox(height: 20,),
                        ElevatedButton(onPressed: (){
                          setState(() {
                           cartItems.add(FishProducts[index]);
                          });
                        ...
                        ElevatedButton(onPressed: (){//favourite 's functionality
                          setState(() {
                           favItems.add(FishProducts[index]);
                          });
                        }, 
